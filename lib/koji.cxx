...
static string_list_t *read_koji_descendent_results(xmlrpc_env *env, xmlrpc_value *value)
{
    ...
    results = (string_list_t *) xalloc(sizeof(*results));
    TAILQ_INIT(results);
...
koji_buildlist_t *init_koji_buildlist(void)
{
    ...
    builds = (koji_buildlist_t *) xalloc(sizeof(*(builds)));
    TAILQ_INIT(builds);
    return builds;
}
...
koji_rpmlist_t *init_koji_rpmlist(void)
{
    ...
    rpms = (koji_rpmlist_t *) xalloc(sizeof(*(rpms)));
    TAILQ_INIT(rpms);
    return rpms;
}
...
void init_koji_task_entry(koji_task_entry_t *entry)
{
    ...
    entry->task = (struct koji_task *) xalloc(sizeof(*entry->task));
    init_koji_task(entry->task);

    entry->brootid = -1;

    entry->srpms = (string_list_t *) xalloc(sizeof(*entry->srpms));
    TAILQ_INIT(entry->srpms);

    entry->rpms = (string_list_t *) xalloc(sizeof(*entry->rpms));
    TAILQ_INIT(entry->rpms);

    entry->logs = (string_list_t *) xalloc(sizeof(*entry->logs));
...
struct koji_build *get_koji_build(struct rpminspect *ri, const char *buildspec)
{
    ...
    build = (struct koji_build *) xalloc(sizeof(*build));
    ...
    while (i < size) {
        ...
        if (!strcmp(keyname, "package_name")) {
            ...
            buildentry = (koji_buildlist_entry_t *) xalloc(sizeof(*buildentry));
            ...
            buildentry->rpms = (koji_rpmlist_t *) xalloc(sizeof(*buildentry->rpms));
    ...
    TAILQ_FOREACH(buildentry, build->builds, builditems) {
        ...
        for (i = 0; i < size; i++) {
            ...
            rpm = (koji_rpmlist_entry_t *) xalloc(sizeof(*rpm));
...
struct koji_task *get_koji_task(struct rpminspect *ri, const char *taskspec)
{
    ...
    /* initialize everything and get XMLRPC ready */
    task = (struct koji_task *) xalloc(sizeof(*task));
    init_koji_task(task);
    xmlrpc_limit_set(XMLRPC_XML_SIZE_LIMIT_ID, SIZE_MAX);
    xmlrpc_env_init(&env);
    xmlrpc_client_init2(&env, XMLRPC_CLIENT_NO_FLAGS, SOFTWARE_NAME, PACKAGE_VERSION, NULL, 0);
    ...
    task->descendents = (koji_task_list_t *) xalloc(sizeof(*task->descendents));
    ...
    for (i = 0; i < size; i++) {
        ...
        for (j = 0; j < dsize; j++) {
            ...
            descendent = (koji_task_entry_t *) xalloc(sizeof(*descendent));
            ...
            for (k = 0; k < rsize; k++) {
                ...
                if (!strcmp(key, "brootid") || !strcmp(key, "buildroot_id")) {
                    ...
                    if (descendent->srpms == NULL) {
                        descendent->srpms = (string_list_t *) xalloc(sizeof(*(descendent->srpms)));
